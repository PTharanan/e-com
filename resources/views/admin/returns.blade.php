...
@section('content')
    <div class="page-header">
        <div class="page-title">
            ...
            <p>Manage product return and refund requests.</p>
        </div>
        <div class="header-actions">
            <form action="{{ route('admin.returns') }}" method="GET" id="filterForm">
                <select name="status" onchange="document.getElementById('filterForm').submit()" class="status-select"
                ...
                @foreach($returns as $return)
                    <tr>
                        ...
                        <td style="font-weight: 600; color: #64748B;">
                            #ORD-{{ str_pad($return->order_id, 5, '0', STR_PAD_LEFT) }}</td>
                        <td>
                        ...
                        <td>
                            <div style="display: flex; gap: 8px;">
                                ...
                                @if($return->status == 'pending')
                                    <button class="status-select" style="color: #EF4444; border-color: #FEE2E2;"
                                        onclick="rejectReturnRequest({{ $return->id }})">Reject</button>
                                @endif
                            </div>
                        </td>
                    </tr>
    ...
    <script>
        ...
        async function assignReturnPartner(id) {
            ...
            try {
                const response = await fetch(`{{ url('admin/returns') }}/${id}/assign`, {
                    method: 'POST',
                    headers: {
        ...
        async function rejectReturnRequest(id) {
            ...
            try {
                const response = await fetch(`{{ url('admin/returns') }}/${id}/status`, {
                    method: 'PATCH',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
                        'Content-Type': 'application/json',
                ...
                if (result.success) {
                    showToast(result.message);
                    location.reload(); // Reload to update UI buttons
                } else {
        ...
        async function updateReturnStatus(id, el) {
            // This is now simplified as we use separate buttons/modals
