...
<style>
    ...
    .seller-profile { display: flex; align-items: center; gap: 12px; }
    .seller-avatar { width: 35px; height: 35px; background: #F1F5F9; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-weight: 700; color: var(--admin-primary); }
    ...
    .status-active { background: #E1F7E3; color: #0E731F; }
    .status-pending { background: #FFF1EE; color: #F25C3B; }
    .status-blocked { background: #FEE2E2; color: #EF4444; }
    ...
    .btn-view:hover { background: var(--admin-primary); color: white; }
</style>
@endsection
            ...
            <tr>
                ...
                <td>
                    <div class="actions-flex">
                        <button class="btn-view" onclick="window.location.href='/admin/products?seller_id={{ $seller->id }}'">
                            View Products
                        </button>
                        <button class="{{ $seller->is_blocked ? 'btn-unblock' : 'btn-block' }}" id="block-btn-{{ $seller->id }}" onclick="toggleBlock({{ $seller->id }})">
