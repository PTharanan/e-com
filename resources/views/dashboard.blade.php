...
@section('styles')
    <style>
        ...
        @keyframes stepPulse {
            ...
            50% {
                box-shadow: 0 0 0 10px rgba(242, 92, 59, 0.05);
            }
        }
        ...
        .timeline-step.completed .step-label {
            color: var(--color-primary, #F25C3B);
        }
                        ...
                        <tbody>
                            @foreach($orders->sortByDesc('created_at') as $order)
                                <tr id="order-row-{{ $order->id }}">
                                    ...
                                    <td data-label="Total Amount" style="font-weight: 700;">
                                        {{ currency_symbol() }}{{ number_format($order->total_price, 2) }}</td>
                                    ...
                                    <td data-label="Secret Code" id="code-container-{{ $order->id }}">
                                        @if($order->status == 'shipped' && $order->delivery?->secret_code)
                                            <div
                                                style="background: #FFFBEB; color: #B45309; padding: 6px 12px; border-radius: 8px; font-weight: 800; font-family: monospace; border: 1px dashed #F59E0B; display: inline-block;">
                                    ...
                                    <td data-label="Proof">
                                                ...
                                                <div style="display: flex; gap: 4px;">
                                                    @if($latestReturn->pickup_image)
                                                        <a href="{{ asset($latestReturn->pickup_image) }}" target="_blank" title="Pickup Proof">
                                                            <img src="{{ asset($latestReturn->pickup_image) }}"
                                                                style="width: 30px; height: 30px; border-radius: 4px; object-fit: cover; border: 1px solid #eee;">
                                                        </a>
                                                    @endif
                                                    @if($latestReturn->store_image)
                                                        <a href="{{ asset($latestReturn->store_image) }}" target="_blank"
                                                            title="Store Return Proof">
                                                            <img src="{{ asset($latestReturn->store_image) }}"
                                                                style="width: 30px; height: 30px; border-radius: 4px; object-fit: cover; border: 1px solid #eee;">
                                                        </a>
                                                    @endif
                                                </div>
                                        ...
                                        @elseif($order->delivery?->delivery_image)
                                            <a href="{{ asset($order->delivery->delivery_image) }}" target="_blank">
                                                <img src="{{ asset($order->delivery->delivery_image) }}"
                                    ...
                                    <td data-label="Action" id="action-container-{{ $order->id }}">
                                        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                                            <button type="button" class="btn-view-details"
                                                onclick="showOrderDetails({{ json_encode($order) }})">
    ...
    @section('scripts')
        <script>
            ...
            function showOrderDetails(order) {
                ...
                const baseUrl = "{{ asset('') }}";
                items.forEach(item => {
                    const itemImg = item.image || item.path;
                    let imgPath = '/placeholder-product.png';
                    if (itemImg) {
                        if (itemImg.startsWith('http')) {
                            imgPath = itemImg;
                        } else if (itemImg.startsWith('/')) {
                            imgPath = baseUrl.replace(/\/$/, '') + itemImg;
                        } else {
                            imgPath = baseUrl + itemImg;
                        }
                    }
