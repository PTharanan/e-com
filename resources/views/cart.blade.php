...
@section('content')
    ...
    <div class="payment-modal-overlay" id="payment-method-modal">
        <div class="payment-card">
            <div class="payment-header">
                <h2>Select Payment Method</h2>
                <button type="button" id="close-method-modal"
                    style="background:none;border:none;font-size:1.5rem;cursor:pointer;">&times;</button>
            </div>
            <div style="display: flex; flex-direction: column; gap: 15px;">
                <button type="button" class="btn-payment btn-pay-confirm" id="btn-choose-stripe">
                    Pay with Stripe
                </button>
                <button type="button" class="btn-payment btn-pay-confirm" id="btn-choose-paypal"
                    style="background: #0070ba; box-shadow: 0 4px 15px rgba(0, 112, 186, 0.3);">
                    Pay with PayPal
                </button>
            </div>
        </div>
    ...
    <div class="payment-modal-overlay" id="payment-modal">
        <div class="payment-card">
            <div class="payment-header stripe-header">
    ...
    <section class="cart-section">
        ...
        <div id="cart-content">
            ...
            <div class="empty-cart-container" id="empty-cart" style="display: none;">
                ...
                <p style="margin: 15px 0 30px; color: var(--color-text-medium);">Looks like you haven't added anything to
                    your cart yet.</p>
                <a href="{{ route('products') }}" class="btn-checkout-page"
                    style="display: inline-block; width: auto; padding: 15px 40px; text-decoration: none;">Start
                    Shopping</a>
            </div>
        </div>
    </section>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const btnChooseStripe = document.getElementById('btn-choose-stripe');
            const btnChoosePayPal = document.getElementById('btn-choose-paypal');
            const closeMethodModal = document.getElementById('close-method-modal');
            ...
            btnChoosePayPal.addEventListener('click', async () => {
                const items = window.cartItems || {};
                if (Object.keys(items).length === 0) return;
                ...
                spinner.classList.add('paypal-style');
                const loadingText = loadingOverlay.querySelector('.loading-text');
                loadingText.innerText = 'Initializing PayPal Payment...';
                loadingOverlay.style.display = 'flex';

                try {
                    const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('{{ route("paypal.checkout") }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
                    ...
                    if (data.approve_link) {
                        ...
                        alert('Error: ' + (data.error || 'Could not initialize PayPal checkout'));
                    }
                } catch (err) {
                    console.error(err);
                    loadingOverlay.style.display = 'none';
            ...
            btnChooseStripe.addEventListener('click', async () => {
                const items = window.cartItems || {};
                if (Object.keys(items).length === 0) return;
                ...
                const loadingOverlay = document.getElementById('loading-overlay');
                const spinner = loadingOverlay.querySelector('.loading-spinner');
                spinner.classList.remove('paypal-style');
                const loadingText = loadingOverlay.querySelector('.loading-text');
                loadingText.innerText = 'Initializing Stripe Payment...';
                loadingOverlay.style.display = 'flex';
                ...
                btnChooseStripe.disabled = false;
                btnChooseStripe.innerText = 'Pay with Stripe';
            });
